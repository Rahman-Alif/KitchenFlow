...
export default function OrderDetailPage() {
  ...
  if (loading) {
    return (
      <KitchenLayout>
        <div className="text-zinc-500 text-sm">Loading order...</div>
      </KitchenLayout>
    );
  }
    ...
    <KitchenLayout>
      <div className="max-w-2xl mx-auto">
        ...
        <button
          ...
          className="text-zinc-400 hover:text-white text-sm mb-6 flex items-center gap-2 transition"
        >
          ← Back to Queue
        </button>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 mb-4">
          <div className="flex justify-between text-sm mb-4">
            ...
            <span className="text-white font-medium">{order.placed_by.name}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-zinc-400">Placed at</span>
            <span className="text-white">{new Date(order.created_at).toLocaleTimeString()}</span>
          </div>
          {order.notes && (
            <div className="mt-4 bg-zinc-800 rounded-lg px-4 py-3 text-sm text-zinc-300">
              📝 {order.notes}
            </div>
          )}
        </div>
        ...
        {/* Transaction — if already recorded */}
        {order.transaction && (
          <div className="bg-green-500/5 border border-green-500/20 rounded-2xl p-6 mb-4">
            <h3 className="text-sm font-semibold text-green-400 uppercase tracking-wider mb-4">
              Payment Recorded
            </h3>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-zinc-400">Tendered</span>
                <span className="text-white">৳{order.transaction.tendered_amount}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-400">Change</span>
                <span className="text-white">৳{order.transaction.change_returned}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {showPayment && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 mb-4">
            ...
            <div className="mb-4">
              ...
              <p className="text-white font-bold text-xl">৳{order.total_amount}</p>
            </div>
            ...
            {change !== null && parseFloat(change) >= 0 && (
              <div className="bg-zinc-800 rounded-lg px-4 py-3 mb-4 flex justify-between text-sm">
          ...
          <div className="flex gap-3">
            ...
            {order.status === 'pending' && (
              <button
                ...
                disabled={actionLoading}
                className="px-4 py-3 bg-zinc-800 hover:bg-red-500/20 hover:text-red-400 text-zinc-300 text-sm rounded-lg transition"
              >
                Cancel Order
              </button>
            )}
